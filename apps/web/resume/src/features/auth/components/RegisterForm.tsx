...
import { registerUserAuthSchema, userAuthPasswordSchema, type RegisterUserAuthType } from '@qiluer-resume/dto/schemas/user-auth';
import { cn } from '@/lib/utils';
import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';
import { Field, FieldDescription, FieldError, FieldGroup, FieldLabel, FieldSeparator } from '@/components/ui/field';
import { Input } from '@/components/ui/input';
import { registerMutationOptions } from '../api/auth.options';
import heroImage from '@assets/hero.png';

const registerFormSchema = registerUserAuthSchema
  .extend({ confirmPassword: userAuthPasswordSchema })
  .refine(({ password, confirmPassword }) => password === confirmPassword, {
    message: '两次输入的密码不一致',
    path: ['confirmPassword'],
  });

type RegisterFormValues = RegisterUserAuthType & { confirmPassword: string };
      ...
      <Card className="overflow-hidden p-0">
        <CardContent className="grid p-0 md:grid-cols-2">
          <form className="p-6 md:p-8" onSubmit={onSubmit} noValidate>
            <FieldGroup>
              <div className="flex flex-col items-center gap-2 text-center">
                ...
                <p className="text-sm text-balance text-muted-foreground">请输入您的电子邮件地址以创建您的账户。</p>
              </div>
              <Field data-invalid={Boolean(errors.email)}>
                ...
                <FieldDescription>我们将利用这一信息来与您联系。我们不会将您的电子邮件地址分享给其他人。</FieldDescription>
              </Field>
              <Field>
                <Field className="grid grid-cols-2 gap-4">
                  ...
                  <Field data-invalid={Boolean(errors.confirmPassword)}>
                    ...
                    />
                    <FieldError id="register-confirm-password-error" errors={[errors.confirmPassword]} />
                  </Field>
                </Field>
              </Field>
              <Field>
          ...
          <div className="relative hidden bg-muted md:block">
            <img src={heroImage} alt="Image" className="absolute inset-0 h-full w-full object-cover dark:brightness-[0.2] dark:grayscale" />
          </div>
        </CardContent>
      </Card>
